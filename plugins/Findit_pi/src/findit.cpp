...
void MainDialog::OnCloseDialog(wxCloseEvent& event)
{
    saveData();
    pPlugin->DeInit();
//    pPlugin->OnDialogClose(); Nicht in 1.2.5
}
...
//    this->m_textCtrl1->SetFocus();  // Such-Funktion aktivieren  Nicht in 1.2.5
...
void MainDialog::OnGridSelectCellMaterial( wxGridEvent& event )
{
    ...
    {
        m_gridMaterial->SetRowMinimalHeight(selGridRow,120);
...
void MainDialog::OnGridSelectCellFood( wxGridEvent& event )
{
    ...
    {
        m_gridFood->SetRowMinimalHeight(selGridRow,120);
...
void MainDialog::OnGridCellChangeLocations( wxGridEvent& event )
{
    ...
    // Testen mit:    wxMessageBox("Text: " + str);
// wxMessageBox(wxString::Format(_("Spalten %ld"),m_data[0].GetCount()));
...
// Wozu??? Macht Crash. Nicht mehr
//        wxMessageBox("Text: " + str);  Wird nicht angesprungen?
        this->m_gridMaterial->SetCellEditor(i,col+LOC1,new wxGridCellChoiceEditor(arr,false));
//        this->m_gridMaterial->SetCellEditor(i,col+LOC1,new wxGridCellChoiceEditor(WXSIZEOF(arr),arr));
    }
...
{
    combo  = new wxGridCellChoiceEditor(location1,false);  // true erlaubt andere Strings (false=Nein)
...
{
//    wxMessageBox(wxString::Format(_("Zeilen %ld"),m_data.GetCount()));  // bis 35-...64-Material
...
//        wxMessageBox(wxString::Format(_("Spalten %ld"),m_data[0].GetCount()));  // 14
...
{
// wird aufgerufen von GetCellValue()
// AssertJan2018: (row < GetNumberRows()) && (col < GetNumberCols())
...
bool myGridStringTable::DeleteRows( size_t pos, size_t numRows )
{
    ...
    if ( pos >= curNumRows )
    {
        wxFAIL_MSG( wxString::Format( wxT("Called myGridStringTable::DeleteRows(pos=%lu, N=%lu)\nPos value is invalid for present table with %lu rows"), (unsigned long)pos, (unsigned long)numRows, (unsigned long)curNumRows ) );
...
bool myGridStringTable::DeleteCols( size_t pos, size_t numCols )
{
    ...
    if ( pos >= curNumCols )
    {
        wxFAIL_MSG( wxString::Format( wxT("Called myGridStringTable::DeleteCols(pos=%lu, N=%lu)\nPos value is invalid for present table with %lu cols"), (unsigned long)pos, (unsigned long)numCols, (unsigned long)curNumCols ) );
        return false;
    }
