...
void MainDialog::setEqualRowHeight(wxGrid* grid, int row)
{
    ...
    if( height > max)
        max = height;
#ifdef __WXOSX__
    grid->SetRowMinimalHeight(row,max);
#else
...
// Wozu??? Macht Crash
//        this->m_gridMaterial->SetCellEditor(i,col+LOC1,new wxGridCellChoiceEditor(WXSIZEOF(arr),false));
	}
...
{
//    wxMessageBox(wxString::Format(_("Zeilen %ld"),m_data.GetCount()));
