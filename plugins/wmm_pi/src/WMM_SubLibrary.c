...
#include <ctype.h>
// #include "wx/wx.h"
// #include "wx/private/wxprintf.h"  // Für wxPrintf ??
#endif
	...
	*/
	...
	switch(control)
	{
		...
		case 22:
			...
			wxPrintf("Please download this file from http://www.ngdc.noaa.gov/geomag/WMM/DoDWMM.shtml.  \n");
			wxPrintf("Replace the existing EGM9615.BIN file with the downloaded one\n");
			break;
#else
		case 1:
			printf("\nError allocating in WMM_LegendreFunctionMemory.\n");
...
// Turn off file loading - we did not use it anyway and linking on Windows was broken...
...
if (Geoid->UseGeoid == 1) {      /* Geoid correction required */
			...
			CoordGeodetic->HeightAboveEllipsoid = CoordGeodetic->HeightAboveGeoid + DeltaHeight / 1000;
			 /*  Input and output should be kilometers,
...
*StartDate, WMMtype_Date *EndDate, int *ElementOption, int *PrintOption, char *OutputFile, WMMtype_Geoid *Geoid)
	...
	*/
	...
	char buffer[20];
	int dummy;
// printf changed to wxPrintf (und Retour)
