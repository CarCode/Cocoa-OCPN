...
br24radar_pi::br24radar_pi(void *ppimgr) : opencpn_plugin_112(ppimgr) {  //  112
...
#ifdef __WXOSX__
    m_shareLocn = *GetpPrivateApplicationDataLocation() + _T("opencpn/plugins") + wxFileName::GetPathSeparator() + _T("br24radar_pi") +
  ...
  double var;
    ...
    if (!wxIsNaN(hdt)) {
        ...
        if (m_heading_source < HEADING_NMEA_HDM) {
            LOG_INFO(wxT("BR24radar_pi: Heading source is now HDM %f + VAR %f from NMEA %s (%d->%d)"), hdm, m_var, sentence.c_str(), m_heading_source, HEADING_NMEA_HDT);
