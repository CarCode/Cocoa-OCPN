...
int br24radar_pi::Init(void)
{
    ...
    if (LoadConfig()) {
#ifdef __WXOSX__
        wxLogMessage(_T("BR24radar_pi: Configuration file values initialised"));
        wxLogMessage(wxString::Format(_T("BR24radar_pi: Log verbosity = %d (to modify, set VerboseLog to 0..4)"), settings.verbose));
#else
...
#ifdef __WXOSX__
        wxLogMessage(_T("BR24radar_pi: configuration file values initialisation failed"));
#else
    ...
    if (r) {
#ifdef __WXOSX__
        wxLogMessage(_T("BR24radar_pi: Unable to create UDP sending socket"));
#else
...
void br24radar_pi::ComputeGuardZoneAngles()
{
    ...
    for (size_t z = 0; z < GUARD_ZONES; z++) {
        switch (guardZones[z].type) {
            case GZ_CIRCLE:
#ifdef __WXOSX__
                wxLogMessage(wxString::Format(_T("BR24radar_pi: GuardZone %d: circle at range %d to %d meters"), z + 1, guardZones[z].inner_range, guardZones[z].outer_range));
#else
            ...
            case GZ_ARC:
#ifdef __WXOSX__
                wxLogMessage(wxString::Format(_T("BR24radar_pi: GuardZone %d: bearing %f to %f range %d to %d meters"), z + 1, guardZones[z].start_bearing, guardZones[z].end_bearing, guardZones[z].inner_range, guardZones[z].outer_range));
#else
                wxLogMessage(wxT("BR24radar_pi: GuardZone %d: bearing %f to %f range %d to %d meters"), z + 1, guardZones[z].start_bearing, guardZones[z].end_bearing, guardZones[z].inner_range, guardZones[z].outer_range);
#endif
            ...
            default:
#ifdef __WXOSX__
                wxLogMessage(_T("BR24radar_pi: GuardZone Off"));
#else
        ...
        for (size_t i = 0; i < LINES_PER_ROTATION; i++) {
            ...
            if (settings.verbose >= 4) {
#ifdef __WXOSX__
                wxLogMessage(wxString::Format(_T("BR24radar_pi: GuardZone %d: angle %f < %f < %f"), z + 1, angle_1, angleDeg, angle_2));
#else
...
#ifdef __WXOSX__
        wxLogMessage(wxString::Format(_T("BR24radar_pi: ComputeGuardZoneAngles done, %d marks"), marks));
#else
