...
/* TR */
	...
	wxMenuItem *pmi = new wxMenuItem(m_pmenu, -1, _("Set Tactics Mark "));
		...
		if (!wxIsNaN(mlat) && !wxIsNaN(mlon) && !wxIsNaN(mCOG) && !wxIsNaN(mHdt)) {
			...
			********************************************************************************************/
            ...
            if (targetTack == _T("R")){ // currently wind is from port ...now
				...
				predictedHdG = (mTWA < 10) ? 180 : 0;
			}
			if (predictedHdG < 0) predictedHdG += 360;
			if (predictedHdG >= 360) predictedHdG -= 360;
			...
			PositionBearingDistanceMercator_Plugin(mlat, mlon, predictedCoG - m_ExpSmoothDegRange / 2., g_dLaylineLengthonChart, &tmplat3, &tmplon3);
			GetCanvasPixLL(vp, &tackpoints[1], tmplat3, tmplon3);
			PositionBearingDistanceMercator_Plugin(mlat, mlon, predictedCoG + m_ExpSmoothDegRange / 2., g_dLaylineLengthonChart, &tmplat4, &tmplon4);
...
#ifdef __WXOSX__
				double cur_tacklinedir=0.0, target_tacklinedir=0.0;
#else
...
//**********************************************
	...
	//--------------------
#ifdef __WXOSX__
    wxStaticText* itemStaticText21 = new wxStaticText(itemPanelNotebook03, wxID_ANY, _("Min. Layline Width [Grad]:"),
#else
	...
	//--------------------
#ifdef __WXOSX__
    wxStaticText* itemStaticText22 = new wxStaticText(itemPanelNotebook03, wxID_ANY, _("Max. Layline Width [Grad]:"),
#else
...
//**********************************************
	...
	//--------------------
#ifdef __WXOSX__
    m_ButtonFixedLeeway = new wxRadioButton(itemPanelNotebook03, wxID_ANY, _("fixed/max Leeway [Grad]:"), wxDefaultPosition,
#else
	m_ButtonFixedLeeway = new wxRadioButton(itemPanelNotebook03, wxID_ANY, _("fixed/max Leeway [\u00B0]:"), wxDefaultPosition,
#endif
        wxDefaultSize, 0);
...
void tactics_pi::CalculateTrueWind(int st, double value, wxString unit)
{
  double spdval;
    ...
    if (st == OCPN_DBP_STC_AWS){
      ...
      if ((!m_bTrueWind_available || g_bForceTrueWindCalculation) && mAWA >= 0 && mAWS >= 0 && spdval >= 0 && mAWAUnit != _T("")) {
        ...
        m_calcTWD = mTWD;
        m_calcTWA = mTWA;
        if (mAWSUnit == _T("")) mAWSUnit = mAWAUnit;
