...
//          pDIB->SelectIntoDC( dc );
        ...
        while( upd.HaveRects() ) {
            ...
            fromSM_Plugin( temp_easting_ul, temp_northing_ul, m_ref_lat, m_ref_lon, &temp_lat_top, &temp_lon_left );
            ...
            fromSM_Plugin( temp_easting_lr, temp_northing_lr, m_ref_lat, m_ref_lon, &temp_lat_bot, &temp_lon_right );
...
#ifdef __WXOSX__
                if( isLight && curLight->hasSectors ) lightsHtml
#else
...
//        free( att_array );  //  Will be freed in ~PI_S57Obj ???
