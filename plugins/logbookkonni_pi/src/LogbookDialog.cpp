...
#endif
#ifdef __WXGTK__
        wxExecute(_T("/bin/bash -c \"") + logbookPlugIn->opt->mailClient +  _T(" -s Logbook-Layout --attach ") + layout + _T("\"\""));
...
void LogbookDialog::setTitleExt()
{
    ...
    {
        case 0:
            titleExt = _(" - Normal Timer - Interval: ")+wxString::Format(_T("%s h %s m"), logbookPlugIn->opt->thour.c_str(),logbookPlugIn->opt->tmin.c_str());
            break;
        case 1:
...
//  logbook->recalculateLogbook(selGridRow);
    ...
    else if(selGridCol == LogbookHTML::WAKE && this->m_notebook8->GetSelection() == 0)
    {
        ...
        logGrids[m_notebook8->GetSelection()]->SetCellValue( selGridRow,selGridCol, (s.IsEmpty()) ? m_menu1->GetLabelText(ev.GetId()) : s + _T("\n") + m_menu1->GetLabelText(ev.GetId()));
        setEqualRowHeight(selGridRow);
...
void LogbookDialog::m_gridGlobalOnGridCellRightClick( wxGridEvent& ev )
{
    ...
    if(ev.GetCol() == 4 && (m_notebook8->GetSelection() == 0))
    {
        ...
        for(unsigned int i = 0; i < ActuellWatch::menuMembers.Count(); i++)
        {
            ...
            this->Connect( item->GetId(), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( LogbookDialog::m_menuItem1OnMenuSelection ) );
        }
    }
    ...
    if(ev.GetCol() == 0 && (m_notebook8->GetSelection() == 0))
    {
        ...
        wxMenuItem *item = new wxMenuItem( m_menu1, SELECT_ROUTE, _("Select Route"), wxEmptyString, wxITEM_NORMAL );
        m_menu1->Prepend( item );
        this->Connect( item->GetId(), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( LogbookDialog::m_menuItem1OnMenuSelection ) );
    }
    ...
    for(wxMenuItemList::iterator it = list.begin(); it != list.end(); it++)
    {
        ...
        this->Disconnect( id, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( LogbookDialog::m_menuItem1OnMenuSelection ) );
    }
}
...
void LogbookDialog::addColdFingerDialog(wxMenu* m_menu)
{
    ...
    wxMenuItem *item = new wxMenuItem( m_menu, COLDFINGER, _("use Textblocks..."), wxEmptyString, wxITEM_NORMAL );
    m_menu1->Prepend( item );
    this->Connect( item->GetId(), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( LogbookDialog::m_menuItem1OnMenuSelection ) );
}
...
wxTreeItemId LogbookDialog::FindMenuItem(int grid, int col, wxString name)
{
    ...
    while (!items.empty())
    {
        ...
        {
            if (((next != tree->GetRootItem() && ((myTreeItem*)tree->GetItemData(next))->grid == grid)) && ((myTreeItem*)tree->GetItemData(next))->gridcol == col && ((myTreeItem*)tree->GetItemData(next))->menu == true)
            {
                ...
                while(next.IsOk())
                {
                    wxMenuItem *item = new wxMenuItem( m_menu1, wxID_ANY, tree->GetItemText(next), wxEmptyString, wxITEM_NORMAL );
                    m_menu1->Prepend( item );
                    this->Connect( item->GetId(), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( LogbookDialog::m_menuItem1OnMenuSelection ) );
...
void LogbookDialog::logSaveOnButtonClick( wxCommandEvent& ev )
{
    ...
    new wxFileDialog(this, _("Save Logbook File"), _T(""), backupFile, filter, wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
...
void LogbookDialog::startNormalTimer()
{
    ...
    {
        if(logbookPlugIn->opt->timerType == 0 && (m_bpButtonTimer->state == 0 || m_bpButtonTimer->state == 1)) timer->Stop();
...
void LogbookDialog::crewSaveOnButtonClick( wxCommandEvent& ev )
{
    ...
    wxFileDialog *saveFileDialog = new wxFileDialog(this, _("Save CrewList File"), _T(""), _T("CrewList"), filter, wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
...
void LogbookDialog::OnKeyDownCrew( wxKeyEvent& ev )
{
    ...
    if((ev.GetKeyCode() != WXK_TAB) || (ev.ShiftDown() && ev.GetKeyCode() != WXK_TAB)) { ev.Skip(); return; }

    if ((ev.ShiftDown() && ev.GetKeyCode() == WXK_TAB) ||
        (ev.GetKeyCode() == WXK_LEFT)) offset = -1;

    if ((!ev.ShiftDown() && ev.GetKeyCode() == WXK_TAB) ||
        (ev.GetKeyCode() == WXK_RIGHT)) offset = 1;
...
void LogbookDialog::OnKeyDownWatch( wxKeyEvent& ev )
{
    ...
    if ((ev.ShiftDown() && ev.GetKeyCode() == WXK_TAB) || (ev.GetKeyCode() == WXK_LEFT)) offset = -1;

    if ((!ev.ShiftDown() && ev.GetKeyCode() == WXK_TAB) || (ev.GetKeyCode() == WXK_RIGHT)) offset = 1;
...
void LogbookDialog::boatSaveOnButtonClick( wxCommandEvent& ev )
{
    ...
    wxFileDialog *saveFileDialog = new wxFileDialog(this, _("Save Boat File"), _T(""), _T("Boat"), filter, wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
...
void LogbookDialog::onButtobClickSaveService(wxCommandEvent & ev)
{
    ...
    wxFileDialog *saveFileDialog = new wxFileDialog(this, _("Save Service File"), _T(""), _T("Service"), filter, wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
...
void LogbookDialog::onGridCellLeftClickService(wxGridEvent &ev)
{
    ...
    if((maintenance->selectedCol == maintenance->WARN || maintenance->selectedCol == maintenance->URGENT) && m_gridMaintanence->GetCellValue(maintenance->selectedRow, maintenance->IF) == maintenance->m_choices[8])
    {
...
void LogbookDialog::onGridCellServiceChange( wxGridEvent& ev )
{
    ...
    if(ev.GetCol() == maintenance->IF|| ev.GetCol() == maintenance->WARN || ev.GetCol() == maintenance->URGENT || ev.GetCol() == maintenance->ACTIVE) maintenance->checkService(m_gridGlobal->GetNumberRows()-1);
}
...
void LogbookDialog::onButtonClickViewService(wxCommandEvent &ev)
{
    if(this->m_radioBtnHTMLService->GetValue())
        maintenance->viewHTML(SERVICE,_T(""), m_choiceSelectLayoutService->GetString(m_choiceSelectLayoutService->GetSelection()),false);
    else
        maintenance->viewODT(SERVICE,_T(""), m_choiceSelectLayoutService->GetString(m_choiceSelectLayoutService->GetSelection()),true);
}
...
void LogbookDialog::onButtobClickSaveRepairs( wxCommandEvent& event )
{
    ...
    wxFileDialog *saveFileDialog = new wxFileDialog(this, _("Save Repairs File"), _T(""), _T("Repairs"), filter, wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
...
void LogbookDialog::onButtonClickViewRepairs( wxCommandEvent& event )
{
    if(this->m_radioBtnHTMLRepairs->GetValue())
        maintenance->viewHTML(REPAIRS,_T(""), m_choiceSelectLayoutRepairs->GetString(m_choiceSelectLayoutRepairs->GetSelection()),false);
    else
        maintenance->viewODT(REPAIRS,_T(""), m_choiceSelectLayoutRepairs->GetString(m_choiceSelectLayoutRepairs->GetSelection()),true);
}
...
void LogbookDialog::onButtobClickSaveBuyParts( wxCommandEvent& event )
{
    ...
    wxFileDialog *saveFileDialog = new wxFileDialog(this, _("Save BuyParts File"), _T(""), _T("BuyParts"), filter, wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
...
void LogbookDialog::onButtonClickViewBuyParts( wxCommandEvent& event )
{
    if(this->m_radioBtnHTMLBuyParts->GetValue())
        maintenance->viewHTML(BUYPARTS,_T(""), m_choiceSelectLayoutBuyParts->GetString(m_choiceSelectLayoutBuyParts->GetSelection()),false);
    else
        maintenance->viewODT(BUYPARTS,_T(""), m_choiceSelectLayoutBuyParts->GetString(m_choiceSelectLayoutBuyParts->GetSelection()),true);
}
...
void LogbookDialog::OnKeyDownBuyParts( wxKeyEvent& ev )
{
    ...
    if (ev.ShiftDown() && ev.GetKeyCode() == WXK_RETURN)
    {
        ...
        if(ev.GetEventObject()->IsKindOf(CLASSINFO(wxTextCtrl))) ctrl->WriteText( wxT("\n") );
        return;
    }
...
void LogbookDialog::OnKeyDownOverview( wxKeyEvent& ev )
{
    ...
    if ((!ev.ShiftDown() && ev.GetKeyCode() == WXK_TAB) || (ev.GetKeyCode() == WXK_RIGHT))
        offset = 1;
...
void LogbookDialog::OnButtonClickOverviewSave( wxCommandEvent& ev )
{
    ...
    wxFileDialog *saveFileDialog = new wxFileDialog(this, _("Save Overview File"), _T(""), _T("Overview"), filter, wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
...
{
    if(m_radioBtnHTMLOverview->GetValue()) overview->viewHTML(_T(""), overviewChoice->GetString(overviewChoice->GetSelection()),false);
    else
        overview->viewODT(_T(""),
...
void LogbookDialog::setDatePattern()
{
    ...
    int i = 0;
    if(s.SubString(i,1) == _T("14")) { LogbookDialog::datePattern = _T("dd") + wxString(dateSeparator); i += 3; }
    if(s.SubString(i,1) == _T("12")) { LogbookDialog::datePattern = _T("mm") + wxString(dateSeparator); i += 3; }
    if(s.SubString(i,3) == _T("2011")) { LogbookDialog::datePattern = _T("yyyy") + wxString(dateSeparator); i += 5; }

    if(s.SubString(i,i+1) == _T("14")) { LogbookDialog::datePattern += _T("dd") + wxString(dateSeparator);  i += 3; }
    if(s.SubString(i,i+1) == _T("12")) { LogbookDialog::datePattern += _T("mm") + wxString(dateSeparator);  i += 3; }
    if(s.SubString(i,i+3) == _T("2011")) { datePattern += _T("yyyy") + wxString(dateSeparator);  i += 5; }

    if(s.SubString(i,i+1) == _T("14")) { LogbookDialog::datePattern += _T("dd");  i += 3; }
    if(s.SubString(i,i+1) == _T("12")) { LogbookDialog::datePattern += _T("mm");  i += 3; }
    if(s.SubString(i,i+3) == _T("2011")) { LogbookDialog::datePattern += _T("yyyy"); /*  i += 5;*/ }  // Not used
}
...
{
    wxCHECK_MSG( (row < GetNumberRows()) && (col < GetNumberCols()), wxEmptyString, _T("invalid row or column index in myGridStringTable") );
...
{
    wxCHECK_RET( (row < GetNumberRows()) && (col < GetNumberCols()), _T("invalid row or column index in myGridStringTable") );
...
{
    wxCHECK_MSG( (row < GetNumberRows()) && (col < GetNumberCols()), true, _T("invalid row or column index in myGridStringTable") );
...
bool myGridStringTable::InsertRows( size_t pos, size_t numRows )
{
    ...
    size_t curNumCols = ( curNumRows > 0 ? m_data[0].GetCount() : ( GetView() ? GetView()->GetNumberCols() : 0 ) );
...
void ColdFinger::init()
{
    ...
    if(!m_treeCtrl3->GetRootItem().IsOk())
    {
        ...
        wxTreeItemId test = this->m_treeCtrl3->AppendItem(menu,dialog->m_gridGlobal->GetColLabelValue(LogbookHTML::REMARKS)+_T(" (")+ dialog->m_notebook8->GetPageText(0)+_T(")"),-fo,-1,item);
        ...
        this->m_treeCtrl3->AppendItem(menu,dialog->m_gridWeather->GetColLabelValue(LogbookHTML::WEATHER)+_T(" (")+ dialog->m_notebook8->GetPageText(1)+_T(")"),fo,-1,item);

        item = new myTreeItem(NODE,_T(""),_T(""),_T(""),_T(""),_T(""),_T(""),1,LogbookHTML::VISIBILITY,false,false,true);
        this->m_treeCtrl3->AppendItem(menu,dialog->m_gridWeather->GetColLabelValue(LogbookHTML::VISIBILITY)+_T(" (")+ dialog->m_notebook8->GetPageText(1)+_T(")"),fo,-1,item);

        item = new myTreeItem(NODE,_T(""),_T(""),_T(""),_T(""),_T(""),_T(""),2,LogbookHTML::SAILS,false,false,true);
        this->m_treeCtrl3->AppendItem(menu,dialog->m_gridMotorSails->GetColLabelValue(LogbookHTML::SAILS)+_T(" (")+ dialog->m_notebook8->GetPageText(2)+_T(")"),fo,-1,item);

        item = new myTreeItem(NODE,_T(""),_T(""),_T(""),_T(""),_T(""),_T(""),2,LogbookHTML::REEF,false,false,true);
        this->m_treeCtrl3->AppendItem(menu,dialog->m_gridMotorSails->GetColLabelValue(LogbookHTML::REEF)+_T(" (")+ dialog->m_notebook8->GetPageText(2)+_T(")"),fo,-1,item);

        item = new myTreeItem(NODE,_T(""),_T(""),_T(""),_T(""),_T(""),_T(""),2,LogbookHTML::MREMARKS,false,false,true);
        this->m_treeCtrl3->AppendItem(menu,dialog->m_gridMotorSails->GetColLabelValue(LogbookHTML::MREMARKS)+_T(" (")+ dialog->m_notebook8->GetPageText(2)+_T(")"),fo,-1,item);
...
// ///////////////// in dialog only ////////////
        ...
        test = this->m_treeCtrl3->AppendItem(menu,dialog->m_gridGlobal->GetColLabelValue(LogbookHTML::REMARKS)+_T(" (")+ dialog->m_notebook8->GetPageText(0)+_T(")"),-fo,-1,item);
...
void ColdFinger::OnMenuTreeSelectionDeleteNodeCold( wxCommandEvent& event )
{
    ...
    if(((myTreeItem*)m_treeCtrl3->GetItemData(selectedItem))->deleteable == false || ((myTreeItem*)m_treeCtrl3->GetItemData(selectedItem))->type == ITEM)
        return;
...
void TimerInterval::OnButtonOKClick( wxCommandEvent& event )
{
    ...
    opt->timerSec = (m_spinCtrlH->GetValue()  * 3600000 + m_spinCtrlM->GetValue() * 60000 + m_spinCtrlS->GetValue() * 1000);
