...
int logbookkonni_pi::Init(void)
{
    ...
    m_leftclick_tool_id  = InsertPlugInTool(_T(""), _img_logbook_pi, _img_logbook_pi, wxITEM_NORMAL, _("Logbook"), _T(""), NULL, LOGBOOK_TOOL_POSITION, 0, this);
...
void logbookkonni_pi::dialogDimmer(PI_ColorScheme cs,wxWindow* ctrl,wxColour col, wxColour col1, wxColour back_color,wxColour text_color, wxColour uitext, wxColour udkrd)
{
    ...
    for(unsigned int i = 0 ; i < kids.GetCount() ; i++)
    {
        ...
        if(win->IsKindOf(CLASSINFO(wxListBox)))
            if(cs == PI_GLOBAL_COLOR_SCHEME_DAY || cs == PI_GLOBAL_COLOR_SCHEME_RGB )
                ((wxListBox*)win)->SetBackgroundColour(wxColour(0,0,0));
            else
                ...
                if(cs == PI_GLOBAL_COLOR_SCHEME_DAY || cs == PI_GLOBAL_COLOR_SCHEME_RGB )
                    ...
                    else if(win->IsKindOf(CLASSINFO(wxButton)))
                    {
                        if(cs == PI_GLOBAL_COLOR_SCHEME_DAY || cs == PI_GLOBAL_COLOR_SCHEME_RGB )
                        {
                            ((wxButton*)win)->SetForegroundColour(wxColour(0,0,0));
                            ((wxButton*)win)->SetBackgroundColour(wxColour(0,0,0));
                        }
                        else
                        {
...
#ifdef __WXMSW__
    optionsDialog = new LogbookOptions(parent, opt, this, -1, _("Logbook Preferences"), wxDefaultPosition,  wxSize( 692,660  ), wxDEFAULT_DIALOG_STYLE|wxRESIZE_BORDER );
#elif defined __WXOSX__
    optionsDialog = new LogbookOptions(parent, opt, this, -1, _("Logbook Preferences"), wxDefaultPosition,  wxSize( 790,685 ) , wxDEFAULT_DIALOG_STYLE|wxRESIZE_BORDER );
#else
    optionsDialog = new LogbookOptions(parent, opt, this, -1, _("Logbook Preferences"), wxDefaultPosition,  wxSize( 740,700 ) , wxDEFAULT_DIALOG_STYLE|wxRESIZE_BORDER );
#endif
...
//            pConf->Write ( _T ( "StatusBar" ), opt->statusbar);
        ...
        if(opt->dtEngine1On.IsValid())
            pConf->Write ( _T ( "Engine1TimeStart" ), opt->dtEngine1On.FormatISODate()+_T(" ")+opt->dtEngine1On.FormatISOTime());
        else
            pConf->Write ( _T ( "Engine1TimeStart" ),wxEmptyString);

        if(opt->dtEngine2On.IsValid())
            pConf->Write ( _T ( "Engine2TimeStart" ), opt->dtEngine2On.FormatISODate()+_T(" ")+opt->dtEngine2On.FormatISOTime());
        else
            pConf->Write ( _T ( "Engine2TimeStart" ),wxEmptyString);

        if(opt->dtGeneratorOn.IsValid())
            pConf->Write ( _T ( "GeneratorTimeStart" ), opt->dtGeneratorOn.FormatISODate()+_T(" ")+opt->dtGeneratorOn.FormatISOTime());
        else
...
void logbookkonni_pi::loadLayouts(wxWindow *parent)
{
    ...
    if ( openFileDialog->ShowModal() == wxID_OK )
    {
        ...
        wxString ok = wxString::Format(_("Layouts %sinstalled at\n\n%s\n%s\n%s\n%s"),(!ret)?n.c_str():wxEmptyString,data.c_str(),data1.c_str(),data2.c_str(),data3.c_str());
...
void logbookkonni_pi::loadLanguages(wxWindow *parent)
{
    ...
    static const wxChar *FILETYPES = _T("LogbookKonni_Languages.zip");
...
#ifdef __WXOSX__
    wxFileDialog* openFileDialog =
    new wxFileDialog( parent, _("Select zipped Languages-Files"), _T(""), FILETYPES, FILETYPES, wxFD_DEFAULT_STYLE, wxDefaultPosition, wxDefaultSize);
#else
    wxFileDialog* openFileDialog =
    new wxFileDialog( parent, _("Select zipped Languages-Files"), _T(""), FILETYPES, FILETYPES, wxFD_OPEN, wxDefaultPosition);
#endif
    if ( openFileDialog->ShowModal() == wxID_OK )
    {
#ifdef __WXOSX__
        ...
        if(!td.Exists(tempdir))
            ::wxMkdir(tempdir);
        wxString cmd= _T("unzip -o ")+openFileDialog->GetPath()+_T(" -d ")+tempdir;
