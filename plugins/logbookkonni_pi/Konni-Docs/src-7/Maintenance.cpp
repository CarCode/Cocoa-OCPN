...
//	grid->SetCellRenderer(lastRow, IF, new wxGridCellChoiceRenderer);
	grid->SetCellEditor(lastRow,IF,new wxGridCellChoiceEditor(12,m_choices,false));

//	grid->SetCellRenderer(lastRow, ACTIVE, new wxGridCellChoiceRenderer);
	grid->SetCellEditor(lastRow,ACTIVE,new wxGridCellChoiceEditor(2,m_YesNo,false));
...
//	if(z.IsEmpty()) return;
	...
	while(!input.Eof())
   {
		...
		while ( tkz.HasMoreTokens() )
		{
			...
			switch(c)
			{
			case TEXT:      s = wxString(s.To8BitData(), wxConvUTF8);
