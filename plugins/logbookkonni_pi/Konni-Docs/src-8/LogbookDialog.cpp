...
void LogbookDialog::onGridCellRepairsSelected( wxGridEvent& ev )
{
	...
	if(ev.GetCol() == maintenance->RPRIORITY)
	{
		...
		sim.MouseClick();
#else
	m_gridMaintanenceRepairs->ShowCellEditControl();
#endif
	}
}
...
#ifdef __WXOSX__
	m_gridMaintanenceRepairs->SetCellValue(event.GetString(),maintenance->selectedRow,maintenance->selectedCol);
	maintenance->checkRepairs();
#endif
...
void LogbookDialog::onGridCellLeftClickBuyParts(wxGridEvent& ev)
{
	...
	if(ev.GetCol() == maintenance->PRIORITY)
	{
		...
		sim.MouseClick();
#else
	m_gridMaintenanceBuyParts->ShowCellEditControl();
#endif
	}
}
...
#ifdef __WXOSX__
	m_gridMaintenanceBuyParts->SetCellValue(event.GetString(),maintenance->selectedRow,maintenance->selectedCol);
	maintenance->checkBuyParts();
#endif
