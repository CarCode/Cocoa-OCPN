...
void Logbook::newLogbook()
{
	...
	int ii = wxMessageBox(_("Reset all Values to zero ?"),_T(""),wxYES_NO );
	if(ii == wxYES)
		zero = true;
    else if(ii == wxNO)
	{ 	dialog->logGrids[dialog->m_logbook->GetSelection()]->SetFocus(); return; }
...
void Logbook::convertTo_1_2()
{
	...
	for(unsigned int i = 0; i < files.Count(); i++)
	{
		...
		wxTextOutputStream* out = new wxTextOutputStream (stream1);
	...
	wxTextOutputStream* out = new wxTextOutputStream (stream2);
	...
	out = new wxTextOutputStream (stream3);
	...
	out = new wxTextOutputStream (stream4);
...
void Logbook::appendRow(bool mode)
{
	...
	if(lastRow >= 499)
	{
		...
		if(lastRow == repeat)
		{
			...
			LinesReminderDlg* dlg = new LinesReminderDlg(str,dialog);
			dlg->Show();
//			wxMessageBox(wxString::Format(_("Your Logbook has %i lines\n\n\You should create a new logbook to minimize loadingtime."),lastRow),_("Information"),wxOK);
		}
...
//                            wxString s = dt.FormatDate().c_str();
                           ...
                           col == LogbookHTML::GENE  || col == LogbookHTML::WATERM)  && !s.IsEmpty()))
					{
						...
						if(wxAtoi(m) > 59)
							{
								wxMessageBox(_("Minutes greater than 59"),_T(""),wxOK);
