...
// wxLogMessage(wxString::Format(_T("** PositionReportTicks %ld %ld %d"), now.GetTicks(), PositionReportTicks, target_age));
        html << vertSpacer
    ...
    if( Class == AIS_GPSG_BUDDY ) {
        ...
        if( Class == AIS_CLASS_A || Class == AIS_CLASS_B || Class == AIS_ARPA || Class == AIS_APRS || Class == AIS_SART) {
            ...
            if( crs < 360 ) {
                ...
                if( g_bShowMag )
                    magString << wxString::Format(wxString("%03d%c(M)"), (int)gFrame->GetMag(crs), 0x00B0);
                if( g_bShowTrue )
...
//                sogStr = wxString::Format( _T("%5.2f ") + getUsrSpeedUnit(), toUsrSpeed( SOG ) );
    ...
    if( b_positionOnceValid && bGPSValid && ( Brg >= 0. ) && ( Range_NM > 0. ) && ( fabs( Lat ) < 85. ) ){
        wxString magString, trueString;
        if( g_bShowMag )
            magString << wxString::Format(wxString("%03d%c(M)"), (int)gFrame->GetMag( Brg ), 0x00B0 );
        if( g_bShowTrue )
...
wxString AIS_Target_Data::GetRolloverString( void )
{
    ...
    if( g_bAISRolloverShowClass || ( Class == AIS_SART ) ) {
        ...
        if( ( Class != AIS_ATON ) && ( Class != AIS_BASE ) ) {
            ...
            if( ( Class != AIS_CLASS_B ) && ( Class != AIS_SART ) && Class != AIS_DSC && !b_SarAircraftPosnReport) {
...
wxString AIS_Target_Data::Get_vessel_type_string( bool b_short )
{
    ...
    if( ( Class == AIS_CLASS_B ) || ( Class == AIS_CLASS_A ) ) {
        ...
        i = ( ShipType == 12  || ShipType == 16) ? 54 : 53;  // 12 is distress
