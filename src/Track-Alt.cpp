...
double Track::Length()
{
    ...
    for(size_t i = 0; i < TrackPoints.size(); i++) {
        ...
        if(l) {
            const double offsetLat = 1e-6;  // 0.000001
//            wxMessageBox("offsetLat: "+ wxString::Format(wxT("%lf"),offsetLat));
            const double deltaLat = l->m_lat - t->m_lat;  // minus oder plus Wert????
//            wxMessageBox("deltaLat: "+ wxString::Format(wxT("%lf"),deltaLat));
            if ( fabs( deltaLat ) > offsetLat ){
                total += DistGreatCircle( l->m_lat, l->m_lon, t->m_lat, t->m_lon );  // Plus Wert
//              wxMessageBox("TotalPlus: "+ wxString::Format(wxT("%lf"),total));
            }else{
                total += DistGreatCircle( l->m_lat + copysign( offsetLat, deltaLat ), l->m_lon, t->m_lat, t->m_lon );  // Minus Wert
//                wxMessageBox("TotalMinus: "+ wxString::Format(wxT("%lf"),total));
            }
        }
        l = t;
    }
// wxMessageBox("Total: "+ wxString::Format(wxT("%lf"),total));
    // 2x Zahlen sonst immer nur 0.000000
    return total;
}
