...
#ifdef __WXOSX__
    Read( _T ( "iENCToolbarX"), &g_iENCToolbarPosX, 0 );
    Read( _T ( "iENCToolbarY"), &g_iENCToolbarPosY, 0 );
#else
...
void SwitchInlandEcdisMode( bool Switch )
{
    if ( Switch ){
        ...
        if (pConfig){
            ...
            int read_int;
#ifdef __WXOSX__
            pConfig->Read( _T ( "nDisplayCategory" ), &read_int, (enum _DisCat) MARINERS_STANDARD );
#else
